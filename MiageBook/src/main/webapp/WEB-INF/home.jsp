...
<div class="jumbotron text-center">
  <c:if test="${not empty sessionScope.moi.pseudo}">
	<p>Bienvenue ${sessionScope.moi.pseudo} <a href="logout">Deconnection</a> </p>
</c:if>
</div>
...
<div class="container">
  <div class="row">
	...
	<c:forEach var="post" items="${requestScope.status}"> 
     	<div class="col-sm-4">
      	<h4>${post.titre}</h4>
     	<p>Mise en ligne par <strong>${post.pseudo}</strong> le ${post.date} <br>
      	<a href="gestion-status?statusId=${post.id}&statusPseudo=${post.pseudo}">ici-details</a></p>
    </div>
    </c:forEach>
	</c:if>
  </div>
